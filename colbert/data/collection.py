...
class Collection:
    def __init__(self, path=None, data=None, do_unidecode: bool = False):
        self.path = path
        self.data = data or self._load_file(path, do_unidecode=do_unidecode)
    ...
    def _load_file(self, path, do_unidecode: bool = False):
        self.path = path
        return (
            self._load_tsv(path, do_unidecode=do_unidecode)
    ...
    def _load_jsonl(self, path) -> List[str]:
        data = file_utils.read_jsonl_file(path)
        collection: List[str] = []
        for datum in data:
            title = datum["title"]
            if title:
                passage = title + " | " + datum["text"]
            else:
                passage = datum["text"]
    ...
    @classmethod
    def cast(cls, obj, **kw_args):
        if type(obj) is str:
            return cls(path=obj, **kw_args)

        if type(obj) is list:
            return cls(data=obj, **kw_args)
