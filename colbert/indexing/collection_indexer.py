...
def encode(
    ...
    shared_queues,
    verbose: int = 3,
    do_unidecode: bool = False,
):
    encoder = CollectionIndexer(
        config=config, collection=collection, verbose=verbose, do_unidecode=do_unidecode
    )
...
class CollectionIndexer:
    ...
    def __init__(
        self, config: ColBERTConfig, collection, verbose=2, do_unidecode: bool = False
    ):
        ...
        self.collection = Collection.cast(collection, do_unidecode=do_unidecode)
        # self.checkpoint = Checkpoint(self.config.checkpoint, colbert_config=self.config)
        # if self.use_gpu:
        #     self.checkpoint = self.checkpoint.cuda()
        self.checkpoint = LightningNewModel.load_from_checkpoint(
    ...
    def _sample_pids(self):
        ...
        typical_doclen = 120  # let's keep sampling independent of the actual doc_maxlen # TODO: Increase the sample size?
...
def compute_faiss_kmeans(
    ...
    assert (
        use_gpu
    ), "Faiss failed to detect GPU. Make sure CUDA is installed and working. If no gpu, remove this assertion."
    kmeans = faiss.Kmeans(
