...
python scripts/preprocess/tokenize_data.py +total=$total +op=split_file +indices=[$begin,$end]
...
for i in $(seq $begin $end); do
    ...
    # Print the command being executed
    echo "Running: CUDA_VISIBLE_DEVICES=$device python scripts/preprocess/tokenize_data.py +total=$total +i=$i +op=tokenize"

    # Run the command with the assigned device and store the output in a text file
    CUDA_VISIBLE_DEVICES=$device python scripts/preprocess/tokenize_data.py +total=$total +i=$i +op=tokenize > output_$i.txt 2>&1 &
done
