...
echo "Running: python scripts/preprocess/split_text_to_sentences.py dataset.name=$dataset_name +total=$total +op=split_file +indices=[$begin,$end]"
python scripts/preprocess/split_text_to_sentences.py dataset.name=$dataset_name +total=$total +op=split_file +indices=[$begin,$end]
...
for i in $(seq $begin $end); do
    ...
    CUDA_VISIBLE_DEVICES=$device python scripts/preprocess/tokenize_data.py dataset.name=$dataset_name +total=$total +i=$i +op=tokenize > output_$i.txt 2>&1 &
done
