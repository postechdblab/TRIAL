...
def tokenize_and_save(
    ...
    tokenized_dataset_path = os.path.join(
        ...
        cfg.dataset.name,
        (
            f"{file_name}.{tokenizer.model_name}-tok.cache"
            if total_proc_num == 1
            else f"{file_name}.{tokenizer.model_name}-tok.{current_proc_idx}.cache"
        ),
    )
    ...
    tok_ids = partial_processor(
        data=[d["text"] for d in dataset],
        func=functools.partial(flat_and_tokenize, tokenizer=tokenizer),
    )

    # Save the tokenized query dataset
    logger.info(f"Saving {len(tok_ids)} tokenized data to {tokenized_dataset_path}")
    save_compressed(tokenized_dataset_path, tok_ids)
    return None


def flat_and_tokenize(texts: List[str], tokenizer: Tokenizer) -> List[int]:
    # Preprocess
