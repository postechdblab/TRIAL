...
def main():
    # Configs
    ckpt_path = "/root/EAGLE/runs/eagle_no_relation_distill_from_author/best_model.ckpt"
    orig_tag = "eagle_no_relation_distill_from_author"
    new_tag = "eagle_no_relation_distill_from_author"

    # Load the checkpoint
    print(f"Loading checkpoint from {ckpt_path}")
    ckpt = torch.load(ckpt_path, weights_only=False)
    # Replace the tag name from orig_tag to new_tag
    ckpt["hyper_parameters"] = check_and_replace_tag(
        ckpt["hyper_parameters"], orig_tag, new_tag
    )
